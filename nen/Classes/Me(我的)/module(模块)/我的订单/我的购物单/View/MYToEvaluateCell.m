...
@implementation MYToEvaluateCell
...
- (void)setModel:(AllOrderModel *)model
{
    ...
    switch (state){
        ...
        case 4:
            [self.PaymentBtn setTitle:@"评价" forState:UIControlStateNormal];
...
- (void)payBtnClick
{
    ...
    NSDictionary *dict = @{@"goodsId":self.otherModel.id,@"orderId":self.model.Id,@"shopId":self.model.shop_id,@"state":self.model.state};
    
       
    [[NSNotificationCenter defaultCenter] postNotificationName:@"toEvaluateRightBtn" object:self userInfo:dict];
}
...
- (void)logisticsBtnClick
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"toEvaluateLeftBtn" object:self userInfo:dict];
