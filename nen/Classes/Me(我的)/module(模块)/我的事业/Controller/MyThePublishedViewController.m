...
- (void)deletePost:(NSNotification *)notification
{
    ...
    // 标题
    self.alertController = [UIAlertController alertControllerWithTitle:@"是否删除帖子!" message:nil preferredStyle:UIAlertControllerStyleAlert];
    // 暂不
...
- (void)pulldownNews
{
    ...
    self.tableView.mj_header = [SHRefreshHeader headerWithRefreshingBlock:^{
        ...
        [MyThePostModel myThePostModelPageSize:@"12" Page:@"1" Type:self.PotType success:^(NSMutableArray<MyThePostModel *> *PostArray) {
...
- (void)pullUpNews
{
    ...
    self.tableView.mj_footer = [SHRefreshFooter footerWithRefreshingBlock:^{
        ...
        NSDictionary *dict = @{@"page":weakSelf.page,@"pagesize":@"12",@"type":self.PotType};
