...
@interface TheLoginPasswordController ()<UITextFieldDelegate>
...
@implementation TheLoginPasswordController
...
- (void)viewDidLoad
{
    ...
    btn.frame = CGRectMake(30,headLabel.sh_bottom + 20,100,90);
    ...
    btn2.frame = CGRectMake(ScreenWidth - 130,btn.sh_y,100,90);
    [btn2 setTitle:@"修改登录密码" forState:UIControlStateNormal];
...
- (void)addCenteriew
{
    ...
    centernView.frame = CGRectMake(15,self.identityBtn.sh_bottom + 20,ScreenWidth - 30,315);
    ...
    verificationLabel.frame = CGRectMake(10,10,centernView.sh_width,25);
    verificationLabel.font = [UIFont systemFontOfSize:15];
    verificationLabel.text = @"请选择验证身份方式";
    [centernView addSubview:verificationLabel];
    
    UILabel *headLabel = [[UILabel alloc] init];
    headLabel.frame = CGRectMake(10,verificationLabel.sh_bottom + 10,centernView.sh_width - 20,25);
    ...
    nextBtn.frame = CGRectMake(20 ,smsTextField.sh_bottom + 10,centernView.sh_width - 40,40);
...
- (void)testingClcik
{
    ...
    NSDictionary *dict= @{@"mobile":self.numberTextFields.text};
...
-(void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    {
        int offset = self.view.frame.origin.y - 105;//iPhone键盘高
        [UIView animateWithDuration:0.5 animations:^{
            ...
            self.headLabels.transform =CGAffineTransformMakeTranslation(0, offset);
    ...
    if (textField == self.smsTextFields)
    {
        [UIView animateWithDuration:0.5 animations:^{
            ...
            self.identityBtn.transform = CGAffineTransformIdentity;
             self.headLabels.transform =CGAffineTransformIdentity;
...
- (void)nextClick
{
    ...
    NSDictionary *dict= @{@"mobile":self.numberTextFields.text,@"type":@"2",@"code":self.smsTextFields.text};
...
- (void)passwordCenter
{
    ...
    passwordTextField.frame = CGRectMake(headLabel.sh_x,headLabel.sh_bottom + 5,passwordCenterView.sh_width - 20,35);
    ...
    againPasswordTextField.frame = CGRectMake(headLabel.sh_x,inputAgainLabel.sh_bottom + 5,passwordCenterView.sh_width - 20,35);
    ...
    submitBtn.frame = CGRectMake(passwordCenterView.sh_width *0.1 ,rightLabel.sh_bottom + 15,passwordCenterView.sh_width *0.8,40);
    [submitBtn setTitle:@"提交" forState:UIControlStateNormal];
    [submitBtn addTarget:self action:@selector(passwordSubmitClick) forControlEvents:UIControlEventTouchUpInside];
...
- (void)passwordSubmitClick
{
    if ([self.passwordTextFields.text isEqualToString:self.againPasswordTextFields.text])
    {
        ...
        // 加密地址
        NSString *splitCompleteStr = [NSString stringEncryptedAddress:@"/mine/setpassword"];
        ...
        NSLog(@"%@",self.codeStr);
        NSDictionary *dict= @{@"mobile":mobileStr,@"password":self.passwordTextFields.text,@"repassword":self.againPasswordTextFields.text,@"code":self.codeStr};
