...
@interface MerchantsToBeProcessedCell()
...
@implementation MerchantsToBeProcessedCell
...
-(void)addSubviewControll
{
    ...
    UIButton *contactSuppliersBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    contactSuppliersBtn.frame = CGRectMake(ScreenWidth - 90,bottomViewOne.sh_bottom + 15,80,23);
    [contactSuppliersBtn setTitle:@"联系供应商" forState:UIControlStateNormal];
    contactSuppliersBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
    contactSuppliersBtn.titleLabel.font = [UIFont systemFontOfSize:13];
    contactSuppliersBtn.layer.cornerRadius = 5;
    [contactSuppliersBtn setTitleColor:[UIColor orangeColor] forState:UIControlStateNormal];
    contactSuppliersBtn.layer.borderColor = [UIColor orangeColor].CGColor;
    contactSuppliersBtn.layer.borderWidth = 2.0f;
    contactSuppliersBtn.clipsToBounds = YES;
    [contactSuppliersBtn addTarget:self action:@selector(contactBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.contentView addSubview:contactSuppliersBtn];
...
- (void)contactBtnClick
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"callSuppliers" object:self];
}
