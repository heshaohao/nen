...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteOrderBtn:) name:@"deleteOrder" object:nil];
    
}

#pragma mark 删除订单
- (void)deleteOrderBtn:(NSNotification *)notification
{
    ...
    NSString *splitCompleteStr = [NSString stringEncryptedAddress:@"/shopmanage/deleteorder"];
    
    NSDictionary *dict = @{@"id":orderId,@"type":@"1"};
    ...
    [manager POST:splitCompleteStr parameters:dict success:^(NSURLSessionDataTask *task, id responseObject) {
      ...
      //  NSLog(@"%@",responseObject);
        ...
        [weakself.merchantsArray enumerateObjectsUsingBlock:^(MerchantsOrdersModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)loadNewData
{
    ...
    [MerchantsOrdersModel merchantsOrdersModelState:@"4" PageSize:@"8" Page:self.page success:^(NSMutableArray<MerchantsOrdersModel *> *merchantsOrdersArray) {
...
- (void)loadMoreData
{
    ...
    NSDictionary *dict = @{@"state":@"4",@"page":self.page,@"pagesize":@"8"};
...
{
    NSString *Id = @"cell";
    HasBeenCompletedCell *cell = [tableView dequeueReusableCellWithIdentifier:Id];
    if (!cell) {
        cell = [[HasBeenCompletedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:Id];
    }
