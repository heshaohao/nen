...
@property(nonatomic,strong) HWCollectionViewCell *colletionViewCell;
...
-(void)initPickerView{
    ...
    //上传图片提示
    addImageStrLabel = [[UILabel alloc]initWithFrame:CGRectMake(120, 50, 70, 20)];
    ...
    addImageStrLabel.font = [UIFont systemFontOfSize:15];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row >=9)
    {
        self.colletionViewCell.hidden = YES;
    }else
    {
        self.colletionViewCell.hidden = NO;
    }
...
{
    return CGSizeMake(([UIScreen mainScreen].bounds.size.width-80) /3 ,([UIScreen mainScreen].bounds.size.width-80) /3);
}
...
- (void)changeCollectionViewHeight{
    ...
    if (_collectionFrameY) {
        _pickerCollectionView.frame = CGRectMake(10, _collectionFrameY, [UIScreen mainScreen].bounds.size.width -20, (((float)[UIScreen mainScreen].bounds.size.width-80.0) /3.0 +20.0)* ((int)(_arrSelected.count)/3 +1)+20.0);
    }
    else{
        _pickerCollectionView.frame = CGRectMake(10, 0, [UIScreen mainScreen].bounds.size.width - 20, (((float)[UIScreen mainScreen].bounds.size.width-80.0) /3.0 +20.0)* ((int)(_arrSelected.count)/3 +1)+20.0);
    }
...
- (void)updatePickerViewFrameY:(CGFloat)Y{
    ...
    _collectionFrameY = Y;
    _pickerCollectionView.frame = CGRectMake(10, Y, [UIScreen mainScreen].bounds.size.width - 20, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /3.0 +20.0)* ((int)(_arrSelected.count)/3 +1)+20.0);
}
