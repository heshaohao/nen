...
@interface FindGoodsController ()<SDCycleScrollViewDelegate>
...
- (void)pulldownNews
{
    ...
    self.tableView.mj_header = [SHRefreshHeader headerWithRefreshingBlock:^{
        ...
        self.page = @"1";
        [ForumPostModel forumPostModelType:@"1" PageSize:@"8" Page:weakself.page success:^(NSMutableArray<ForumPostModel *> *forumPosArray) {
...
- (void)pullUpNews
{
    self.tableView.mj_footer = [SHRefreshFooter footerWithRefreshingBlock:^{
        ...
        NSDictionary *dict = @{@"page":weakself.page,@"pagesize":@"12",@"type":@"1"};
...
#pragma mark 点赞按钮
- (void)thumbBtnClick:(NSNotification *)notification
{
    NSString *group = [NSString stringWithFormat:@"%@",notification.userInfo[@"findGroup"]];
    NSString *row = [NSString stringWithFormat:@"%@",notification.userInfo[@"findRow"]];
    
    
    NSIndexPath *indexPath=[NSIndexPath indexPathForRow:[row integerValue] inSection:[group integerValue]];
...
{
    [self findPushCommentsViewControllerPostId:[NSString stringWithFormat:@"%@",notification.userInfo[@"id"]]];
    
}

// 跳转评论控制器
- (void)findPushCommentsViewControllerPostId:(NSString *)postId
{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self findPushCommentsViewControllerPostId:[NSString stringWithFormat:@"%@",self.forumPostArray[indexPath.row].id]];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // 点赞按钮
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(thumbBtnClick:) name:@"thumbBtn" object:nil];
    // 评论按钮
    //
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(commentBtnClick:) name:@"commentBtn" object:nil];
