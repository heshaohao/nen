...
@interface MarketingCompanyController ()<SDCycleScrollViewDelegate>
// 图片轮播器
...
@implementation MarketingCompanyController
...
- (void)viewDidLoad {
    ...
    // 点赞按钮
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(marketingThumbBtnClick) name:@"marketingThumbBtn" object:nil];
    // 评论按钮
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(marketingCommentBtnClick:) name:@"marketingCommentBtn" object:nil];
...
- (void)pulldownNews
{
    ...
    self.tableView.mj_header = [SHRefreshHeader headerWithRefreshingBlock:^{
        ...
        [ForumPostModel forumPostModelType:@"2" PageSize:@"12" Page:self.page success:^(NSMutableArray<ForumPostModel *> *forumPosArray) {
...
- (void)pullUpNews
{
    ...
    self.tableView.mj_footer = [SHRefreshFooter footerWithRefreshingBlock:^{
        ...
        NSDictionary *dict = @{@"page":weakself.page,@"pagesize":@"12",@"type":@"2"};
...
#pragma mark 点赞按钮
- (void)marketingThumbBtnClick
{
    //    [self.tableView reloadData];
}

#pragma mark 评论按钮
- (void)marketingCommentBtnClick:(NSNotification *)notification
{
   [self marketingPushCommentsViewControllerPostId:[NSString stringWithFormat:@"%@",notification.userInfo[@"id"]]];
}

// 跳转评论控制器
- (void)marketingPushCommentsViewControllerPostId:(NSString *)postId
{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self marketingPushCommentsViewControllerPostId:[NSString stringWithFormat:@"%@",self.forumPostArray[indexPath.row].id]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // 1.确定重用标示:
    static NSString *ID = @"MarketingCell";
