...
@interface InnovativeCompaniesController ()<SDCycleScrollViewDelegate>
// 图片轮播器
...
@implementation InnovativeCompaniesController
...
- (void)viewDidLoad {
    ...
    // 点赞按钮
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(innovativeThumbBtnClick) name:@"innovativeThumbBtn" object:nil];
    // 评论按钮
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(innovativeCommentBtnClick:) name:@"innovativeCommentBtn" object:nil];
...
- (void)pulldownNews
{
    ...
    self.tableView.mj_header = [SHRefreshHeader headerWithRefreshingBlock:^{
        ...
        [ForumPostModel forumPostModelType:@"4" PageSize:@"8" Page:self.page success:^(NSMutableArray<ForumPostModel *> *forumPosArray) {
...
- (void)pullUpNews
{
    ...
    self.tableView.mj_footer = [SHRefreshFooter footerWithRefreshingBlock:^{
        ...
        NSDictionary *dict = @{@"page":weakself.page,@"pagesize":@"8",@"type":@"4"};
...
#pragma mark 点赞按钮
- (void)innovativeThumbBtnClick
{
    //    [self.tableView reloadData];
}

#pragma mark 评论按钮
- (void)innovativeCommentBtnClick:(NSNotification *)notification
{
    
    PostCommentsViewController*postVc = [[PostCommentsViewController alloc] init];
    postVc.postId = notification.userInfo[@"id"];
    [self.navigationController pushViewController:postVc animated:YES];
    
}

// 跳转评论控制器
- (void)innovativeCommentsViewControllerPostId:(NSString *)postId
{
    PostCommentsViewController*postVc = [[PostCommentsViewController alloc] init];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self innovativeCommentsViewControllerPostId:[NSString stringWithFormat:@"%@",self.forumPostArray[indexPath.row].id]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // 1.确定重用标示:
    static NSString *ID = @"InnovativeCell";
