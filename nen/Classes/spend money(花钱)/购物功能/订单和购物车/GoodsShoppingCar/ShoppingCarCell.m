...
- (void)addBtnClick:(UIButton*)button {
    ...
    NSString *completeStr = [NSString stringEncryptedAddress:@"/order/updategoodscart"];
    
     NSDictionary *dict = @{@"id":self.selectId,@"oper":@"add"};
...
- (void)cutBtnClick:(UIButton*)button {
    ...
    NSDictionary *dict = @{@"id":self.selectId,@"oper":@"sub"};
...
-(void)setupMainView {
   ...
   // CGFloat width = (bgView.sh_width - imageBgView.sh_right - 30)/2.0;
    ...
    priceLabel.frame = CGRectMake(nameLabel.sh_x, addBtn.sh_y, 100, 30);
